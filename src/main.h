...
static const int64_t MIN_MIXER_FEE = .01 * COIN;
...
public:
    ...
    {
        // Fibre: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Fibre: get transaction coin age
...
public:
    ...
    // Fibre: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Fibre: two types of block: proof-of-work or proof-of-stake
    ...
    // Fibre: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Fibre: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Fibre: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Fibre: block index flags
    enum  
    {
